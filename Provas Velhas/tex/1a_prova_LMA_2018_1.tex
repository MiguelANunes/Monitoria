...
\begin{center}
\sf
$1^{\underline{a}}$ Avaliação de Lógica Matemática  (LMA)\\
Professores: Claudio ($T_A$) e Jeferson  ($T_B$) \\
Joinville, \today
\end{center}
\end{minipage}
} %% 
...
%\item $(\sim p \wedge \sim q) \leftrightarrow  (p \vee q) $
\item $(\sim p \vee \sim q) \leftrightarrow  (p \wedge q) $
%\item $p \vee q \rightarrow ~~(q \oplus p)$
...
%\item $(p \wedge q) \leftrightarrow (\sim p \vee \sim q)$ %% pelo menos uma de 8 linhas
...
%\textcolor{red}{Deixo esta daqui para voce modificar .... escolha alguma abaixo ou pegue do site}
\begin{enumerate}
\setlength{\itemsep}{-2pt}
...
\item[Identidade (IDENT):] $P \vee \blacksquare \Leftrightarrow \blacksquare$ ou $P \vee \square \Leftrightarrow P$ ou $P \wedge \blacksquare \Leftrightarrow P$ ou $P \wedge \square \Leftrightarrow \square$ 
